...
describe("channelActionSchema", () => {
  it("accepts exact target-state channel actions", () => {
    expect(
      channelActionSchema.parse({ action: "pin", pinned: true }),
    ).toEqual({ action: "pin", pinned: true });
    expect(
      channelActionSchema.parse({ action: "mute", muted: false }),
    ).toEqual({ action: "mute", muted: false });
    expect(
